...
OUTPUT_DIR = "/home/pilot/Desktop/ballsaction/foul_dataset"
# We target a 1:4.5 imbalance (1 highlight for every 4.5 backgrounds)
BG_RATIO = 4.5

HIGHLIGHT_CLASSES = ["Foul", "Indirect free-kick", "Direct free-kick", "Penalty"]
HARD_NEGATIVE_CLASSES = ["Kick-off", "Clearance", "Corner", "Throw-in", "Substitution", "Yellow card", "Red card", "Offside", "Ball out of play"]
...
def process_game(args):
    ...
    print(f"[{game_idx}] {game} | Highlights: {highlights_extracted} | Backgrounds: {bg_extracted} ({hn_extracted} HN)")
    return highlights_extracted, bg_extracted
...
def main():
    ...
    print(f"Found {len(downloaded_games)} games on disk. Beginning extraction with ThreadPoolExecutor...")
    
    total_highlights_extracted = 0
    total_background_extracted = 0
    
    args = [(i, game) for i, game in enumerate(downloaded_games)]
    
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        results = executor.map(process_game, args)
        
        for h_count, b_count in results:
            total_highlights_extracted += h_count
            total_background_extracted += b_count
