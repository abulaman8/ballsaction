...
def train():
    ...
    data_dir = "/home/pilot/Desktop/ballsaction/x3d_dataset"
    ...
    if len(train_dataset) == 0:
        print("Dataset is empty. Run extract_x3d_sequences.py first.")
        return
    ...
    # We extracted roughly 4.5 background for every 1 highlight
    class_weights = torch.tensor([1.0, 4.5], dtype=torch.float32).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), "checkpoints/x3d_best.pth")
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), f"checkpoints/x3d_epoch_{epoch+1}.pth")
            print(f"  [*] Periodic Checkpoint Saved: x3d_epoch_{epoch+1}.pth")
